namespace MvcTemplate.Services.Data
{
    ...
    public class UserService : IUserService
    {
        ...
        {
            if (this.WasBookOnShelf(book, user))
            {
                var relation = this.userBooks.AllWithDeleted().First(x => x.BookId == book.Id && x.ApplicationUserId == user.Id);
                relation.IsDeleted = false;
                relation.DeletedOn = null;
            }
            else
            {
        ...
        private bool WasBookOnShelf(Book book, ApplicationUser user)
        {
            return this.userBooks.AllWithDeleted().Any(x => x.BookId == book.Id && x.ApplicationUserId == user.Id);
        }
    }
}
